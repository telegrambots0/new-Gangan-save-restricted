    ...
    for chunk in chunks:
        ...
        for id_num in chunk:
            ...
            try:
                ...
                if int(fw.value) > 10:
                    await client.send_message(sender, f'You have floodwaits of {fw.value + 5} seconds, cancelling batch')
                    ids_data.pop(str(sender))
                    return -1, True
                else:
                    fw_alert = await client.send_message(sender,
                                                         f'Sleeping for {fw.value + 5} second(s) due to Telegram floodwait.')
                    ors = fw.value + 5
                ...
                if countdown.text != count_down:
                    await countdown.edit(count_down,
                                         buttons=[[Button.url("Join Channel", url="https://t.me/SmexyStore")]])
        ...
        if processed_ids % batch_size == 0:
            ...
            await countdown.edit(f"**Batch process ongoing...**\n\nProcessed: {processed_ids} Links", buttons=[[Button.url("Join Channel", url="http://t.me/SmexyStore")]])
